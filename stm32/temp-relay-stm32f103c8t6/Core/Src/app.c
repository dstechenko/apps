...
void ReadTemperatureFrom(const uint8_t reg, uint8_t buf[20])
{
	...
	buf[0] = reg;
	ret = HAL_I2C_Master_Transmit(&hi2c1, TMP102_ADDR_READ, buf, 1,
				      HAL_MAX_DELAY);
	if (ret != HAL_OK) {
	...
	ret = HAL_I2C_Master_Receive(&hi2c1, TMP102_ADDR_READ, buf, 2,
				     HAL_MAX_DELAY);
	if (ret != HAL_OK) {
...
void WriteTemperatureTo(const uint8_t reg, const float temp, uint8_t buf[20])
{
	...
	ret = HAL_I2C_Master_Transmit(&hi2c1, TMP102_ADDR_WRITE, buf, 3,
				      HAL_MAX_DELAY);
	if (ret != HAL_OK) {
		return;
	}
...
void App_StartDefaultTask(void)
{
	...
	while (true) {
		...
		HAL_UART_Transmit(&huart1, buf, strlen((char *)buf),
				  HAL_MAX_DELAY);
		ReadTemperatureFrom(TMP102_REG_TCUR, buf);
		HAL_UART_Transmit(&huart1, buf, strlen((char *)buf),
				  HAL_MAX_DELAY);
